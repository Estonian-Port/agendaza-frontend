...
export class ExtraService {
  ...
  async getAllExtraByEmpresaId(pageNumber : number) {
    const listaItem$ = this.httpClient.get<ExtraJSON[]>(REST_SERVER_URL + '/getAllExtra/' + this.usuarioService.getEmpresaId() + '/' + pageNumber)
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => Extra.fromJson(extra))
  }

  async getAllExtraByFilterName(pageNumber : number, buscar : string) {
    const listaItem$ = this.httpClient.get<ExtraJSON[]>(REST_SERVER_URL + '/getAllExtraFilter/' + this.usuarioService.getEmpresaId() + '/' + pageNumber + '/' + buscar)
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => Extra.fromJson(extra))
  }
  ...
  async getAllExtraCateringByEmpresaId(pageNumber : number) {
    const listaItem$ = this.httpClient.get<ExtraJSON[]>(REST_SERVER_URL + '/getAllExtraCatering/' + this.usuarioService.getEmpresaId() + '/' + pageNumber)
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => Extra.fromJson(extra))
  }

  async getAllExtraCateringFilter(pageNumber : number, buscar : string) {
    const listaItem$ = this.httpClient.get<ExtraJSON[]>(REST_SERVER_URL + '/getAllExtraCateringFilter/' + this.usuarioService.getEmpresaId() + '/' + pageNumber + '/' + buscar)
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => Extra.fromJson(extra))
  }
  ...
  delete(id: number) {
    return this.httpClient.delete<GenericItem>(REST_SERVER_URL + '/deleteExtra/' + id + "/" + this.usuarioService.getEmpresaId())
  }

  async getAllPrecioConFechaByExtraId(extraId: number) {
    const listaItem$ = this.httpClient.get<PrecioJSON[]>(REST_SERVER_URL + '/getAllPrecioConFechaByExtraId/' + this.usuarioService.getEmpresaId() + '/' + extraId)
    const listaItem = await lastValueFrom(listaItem$)
  ...
  async getAllExtraEventoByTipoEventoIdAndFecha(id : number, fechaInicio : FechaForm) {
    const listaItem$ = this.httpClient.put<Extra[]>(REST_SERVER_URL + '/getAllExtraEventoConPrecioByTipoEventoAndFecha/' + this.usuarioService.getEmpresaId() + '/' + id, new Date(fechaInicio.year, fechaInicio.mes, fechaInicio.dia))
    return await lastValueFrom(listaItem$)
  }

  async getAllExtraEventoVariableByTipoEventoIdAndFecha(id : number, fechaInicio : FechaForm) {
    const listaItem$ = this.httpClient.put<ExtraVariable[]>(REST_SERVER_URL + '/getAllExtraEventoVariableByTipoEventoIdAndFecha/' + this.usuarioService.getEmpresaId() + '/' + + id, new Date(fechaInicio.year, fechaInicio.mes, fechaInicio.dia))
    return await lastValueFrom(listaItem$)
  }

  // TODO Reemplazar fechaForm en getAllTipo...
  async getAllTipoCateringByTipoEventoIdAndFecha(id : number, fechaInicio : FechaForm) {
    const listaItem$ = this.httpClient.put<Extra[]>(REST_SERVER_URL + '/getAllTipoCateringByTipoEventoIdAndFecha/' + this.usuarioService.getEmpresaId() + '/' + id, new Date(fechaInicio.year, fechaInicio.mes, fechaInicio.dia))
    return await lastValueFrom(listaItem$)
  }

  async getAllCateringExtraByTipoEventoIdAndFecha(id : number, fechaInicio : FechaForm) {
    const listaItem$ = this.httpClient.put<ExtraVariable[]>(REST_SERVER_URL + '/getAllCateringExtraByTipoEventoIdAndFecha/' + this.usuarioService.getEmpresaId() + '/' + + id, new Date(fechaInicio.year, fechaInicio.mes, fechaInicio.dia))
    return await lastValueFrom(listaItem$)
  }

  async getAllExtraEventoAgregar() {
    const listaItem$ = this.httpClient.get<Extra[]>(REST_SERVER_URL + '/getAllExtraEventoAgregar/' + this.usuarioService.getEmpresaId())
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => GenericItem.fromJson(extra))
  }

  async getAllExtraCateringAgregar() {
    const listaItem$ = this.httpClient.get<Extra[]>(REST_SERVER_URL + '/getAllExtraCateringAgregar/' + this.usuarioService.getEmpresaId())
    const listaItem = await lastValueFrom(listaItem$)
    return listaItem.map((extra) => GenericItem.fromJson(extra))
  }
